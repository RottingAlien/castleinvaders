...
public class RunnerSkeleton extends Enemy {
    ...
    // -----------------------------------------------------------------------------------------------------------------
    public RunnerSkeleton(int hearts, int randomSpawn, int randomX) {
        super(hearts);

        this.setPic(new Picture());
        this.getPic().grow(10, 10);
        this.getPic().draw();
    ...
    public void punchDoor(Door door) {
        if (getPic().getX() == door.getPic().getX()) {
            door.hit(Globals.PUNCH_WEAPON_DAMAGE + 10);
            System.out.println("Runner hitted the door");
